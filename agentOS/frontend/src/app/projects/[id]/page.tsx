...
export default function ProjectPage() {
  ...
  // Check if context is saved (imported)
  ...
  async function handleImport(e: React.FormEvent) {
    ...
    try {
      ...
      setTimeout(() => { setShowImport(false); setImportStatus(""); }, 3000);
    } catch (err: unknown) {
  ...
  async function handleGenerateSkills() {
    ...
    setGenStatus("Generating skills & workflows from saved context...");
    try {
      const result = await post<SkillsResult>(`api/projects/${id}/generate-skills`, {});
      setGenStatus(`Created ${result.skills_created} skills and ${result.workflows_created} workflows`);
      fetchSkills();
      fetchWorkflows();
    } catch (err: unknown) {
      const msg = err && typeof err === "object" && "message" in err ? (err as { message: string }).message : "Generation failed";
  ...
  if (loadingProject) {
    return <div className="flex h-screen items-center justify-center text-sm text-[#999]">Loading...</div>;
  }
  if (!project) {
    return (
  ...
  return (
    <div className="min-h-screen bg-white">
            ...
            {hasContext && (
              <>
                <button
                  ...
                  className="bg-[#9d66ff] px-4 py-2 text-sm font-medium text-white hover:bg-[#7b3aed] disabled:opacity-50"
                >
                ...
                <button
                  ...
                  className="bg-[#0d0d0d] px-4 py-2 text-sm font-medium text-white hover:bg-[#333] disabled:opacity-50"
                >
      ...
      {/* Status bar */}
      {genStatus && (
        <div className={`border-b border-[#e2e2e2] px-8 py-3 text-sm ${genStatus.startsWith("Error") ? "bg-red-50 text-red-700" : "bg-[#f0e6ff] text-[#7b3aed]"}`}>
          {genStatus}
        </div>
      )}
      ...
      {claudeMD && (
        <div className="border-b border-[#e2e2e2] px-8 py-4">
          <div className="flex items-center justify-between pb-3">
            ...
            <button
              onClick={() => navigator.clipboard.writeText(claudeMD)}
      ...
      <div className="flex">
        ...
        <div className="flex-1 border-r border-[#e2e2e2]">
          <div className="flex items-center justify-between border-b border-[#e2e2e2] px-8 py-4">
            ...
            <button onClick={() => setShowAddSkill(true)} className="bg-[#0d0d0d] px-3 py-1.5 text-xs font-medium text-white hover:bg-[#333]">+ Add Skill</button>
          </div>
          {showAddSkill && (
            <form onSubmit={handleCreateSkill} className="border-b border-[#e2e2e2] px-8 py-4">
              <div className="flex gap-3">
                <input type="text" value={skillName} onChange={(e) => setSkillName(e.target.value)} required placeholder="Skill name" className="flex-1 border border-[#e2e2e2] px-3 py-2 text-sm outline-none focus:border-[#9d66ff]" />
                <select value={skillType} onChange={(e) => setSkillType(e.target.value as SkillType)} className="border border-[#e2e2e2] px-3 py-2 text-sm outline-none">
          ...
          <div className="px-8">
            {skills.length === 0 ? (
              <p className="py-10 text-center text-sm text-[#999]">No skills yet — import a repo and generate, or add manually</p>
            ) : skills.map((skill) => {
              ...
              return (
                <div key={skill.id} className="flex items-center justify-between border-b border-[#e2e2e2] py-3">
                  <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => deleteSkill(skill.id)} className="text-xs text-[#999] hover:text-red-600">Delete</button>
                </div>
              );
            })}
          </div>
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between border-b border-[#e2e2e2] px-8 py-4">
            ...
            <button onClick={() => setShowAddWorkflow(true)} className="bg-[#0d0d0d] px-3 py-1.5 text-xs font-medium text-white hover:bg-[#333]">+ Add Workflow</button>
          </div>
          {showAddWorkflow && (
            <form onSubmit={handleCreateWorkflow} className="border-b border-[#e2e2e2] px-8 py-4">
              <div className="flex gap-3">
                <input type="text" value={workflowName} onChange={(e) => setWorkflowName(e.target.value)} required placeholder="Workflow name" className="flex-1 border border-[#e2e2e2] px-3 py-2 text-sm outline-none focus:border-[#9d66ff]" />
                <button type="submit" className="bg-[#0d0d0d] px-4 py-2 text-xs font-medium text-white hover:bg-[#333]">Create</button>
          ...
          <div className="px-8">
            {workflows.length === 0 ? (
              <p className="py-10 text-center text-sm text-[#999]">No workflows yet — import a repo and generate, or add manually</p>
            ) : workflows.map((wf) => (
              <Link key={wf.id} href={`/projects/${id}/workflows/${wf.id}`} className="group flex items-center justify-between border-b border-[#e2e2e2] py-3">
                <div className="flex items-center gap-3">
                  <div className="flex size-7 items-center justify-center bg-[#c5ebff]"><span className="text-xs font-bold text-[#0ea5e9]">W</span></div>
                  <span className="text-sm font-medium text-[#0d0d0d] group-hover:text-[#7b3aed]">{wf.name}</span>
                  <span className="text-[10px] text-[#999]">v{wf.version}</span>
                </div>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#ccc" strokeWidth="2" className="group-hover:stroke-[#7b3aed]"><path d="M5 12h14" /><path d="M12 5l7 7-7 7" /></svg>
              </Link>
            ))}
          </div>
        </div>
          ...
          <form onSubmit={handleImport} className="w-full max-w-lg border border-[#e2e2e2] bg-white shadow-2xl">
            ...
            <div className="flex flex-col gap-4 px-6 py-5">
              <div>
                ...
                <input id="import-url" type="text" value={importURL} onChange={(e) => setImportURL(e.target.value)} required placeholder="https://github.com/owner/repo" className="mt-1.5 w-full border border-[#e2e2e2] px-4 py-2.5 text-sm text-[#0d0d0d] placeholder-[#999] outline-none focus:border-[#9d66ff]" />
              </div>
              {importStatus && (
            ...
            <div className="flex justify-end gap-3 border-t border-[#e2e2e2] px-6 py-4">
              <button type="button" onClick={() => { setShowImport(false); setImportStatus(""); }} className="border border-[#e2e2e2] px-5 py-2 text-sm text-[#4d4d4d] hover:bg-[#f5f5f5]">Cancel</button>
              <button type="submit" disabled={importing} className="bg-[#0d0d0d] px-5 py-2 text-sm font-medium text-white hover:bg-[#333] disabled:opacity-50">{importing ? "Extracting context..." : "Import & Extract Context"}</button>
            </div>
          </form>
        </div>
      )}
